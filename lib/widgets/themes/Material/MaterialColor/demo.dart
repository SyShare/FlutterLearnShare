...
class PaletteTabView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle whiteTextStyle =
        textTheme.bodyMedium?.copyWith(color: Colors.white) ?? TextStyle(
          color: Colors.white
        );
    final TextStyle blackTextStyle =
        textTheme.bodyMedium?.copyWith(color: Colors.black) ?? TextStyle(
            color: Colors.black
        );;
    ...
    if (colors.accent != null) {
      colorItems.addAll(accentKeys.map<Widget>((int index) {
        return DefaultTextStyle(
          style: index > colors.threshold ? whiteTextStyle : blackTextStyle,
          child:
              ColorItem(index: index, color: colors.accent?? Colors.red, prefix: 'A'),
        );
      }).toList());
    }

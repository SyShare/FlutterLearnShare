...
import 'package:flutter_one/widgets/components/List/AnimatedList/model.dart';
...
class _AnimatedListFullDefault extends State<AnimatedListFullDefault> {
  ...
  int? _selectedItem = 0;
  int _nextItem = 1; // The next item inserted when the user presses the '+' button.
  ...
  void insert() {
    final int index =
        _selectedItem == null ? _list.length : _list.indexOf(_selectedItem ?? 0);
...
class CardItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    TextStyle textStyle = Theme.of(context).textTheme.bodyText1 ?? const TextStyle(
      color: Colors.red
    );
    if (selected)

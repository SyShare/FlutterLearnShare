...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  MarkdownBuilder(
      {this.delegate,required this.styleSheet,required this.imageDirectory, this.demoParser});
  ...
  List<Widget> _mergeInlineChildren(_InlineElement inline) {
    ...
    for (Widget child in inline.children) {
      if (mergedTexts.isNotEmpty &&
        ...
        TextSpan? textSpan  = previous.text as TextSpan?;
        List<TextSpan>? spans = textSpan?.children as List<TextSpan>?;
        List<TextSpan> children = (spans != null)
            ? new List.from(spans)
            : [];
        if(textSpan != null)
